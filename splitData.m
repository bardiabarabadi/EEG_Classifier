...
function [adsTrain,adsValidation] = splitData(ads,datafolder)

% Read the list of validation files
if isfile(fullfile(datafolder,'validation_list.txt'))
    ...
    files = ads.Files;
    isTest=false(size(files));
    isValidation=~logical(randi([0,5], size(files,1),1));
    adsValidation = subset(ads,isValidation);
